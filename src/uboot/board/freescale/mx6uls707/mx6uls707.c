...
#define LCD_PIXCLK_PAD_CTRL    (PAD_CTL_HYS  | PAD_CTL_PUE | \
	PAD_CTL_PKE | PAD_CTL_SPEED_HIGH | PAD_CTL_DSE_120ohm)
...
static iomux_v3_cfg_t const lcd_pads[] = {
	MX6_PAD_LCD_CLK__LCDIF_CLK | MUX_PAD_CTRL(LCD_PIXCLK_PAD_CTRL),
	MX6_PAD_LCD_ENABLE__LCDIF_ENABLE | MUX_PAD_CTRL(LCD_PAD_CTRL),
	MX6_PAD_LCD_HSYNC__LCDIF_HSYNC   | MUX_PAD_CTRL(LCD_PIXCLK_PAD_CTRL),
	MX6_PAD_LCD_VSYNC__LCDIF_VSYNC   | MUX_PAD_CTRL(LCD_PIXCLK_PAD_CTRL),
	...
	/* Use GPIO for Brightness adjustment, low open,high turn off */
...
struct display_info_t const displays[] = {{
	...
	.mode	= {
		...
		.xres           = 800,
		.yres           = 800,
		.pixclock       = 20833,//14065,//71.1MHz   20833:48MHz
		.left_margin	= HBPD,   // HBPD(horizontal back porch)
		.right_margin	= HFPD, // HFPD(horizontal front porth)
		.upper_margin	= VBPD,   // VBPD(vertical back porch)
		.lower_margin	= VFPD,   // VFBD(vertical front porch)
		.hsync_len		= HSPW, // HSPW(horizontal sync pulse width)
		.vsync_len		= VSPW, // VSPW(vertical sync pulse width)
		.sync			= 0,
...
{
	if (is_mx6ul_9x9_evk())
		puts("Board: MX6UL 9x9 S707\n");
	else
		puts("Board: MX6UL 14x14 S707\n");
